/**
 * @file   laplace_operator_3d.cc
 *
 * @author Lars Pastewka <lars.pastewka@imtek.uni-freiburg.de>
 *
 * @date   01 Jan 2026
 *
 * @brief  Host implementation of hard-coded 3D Laplace operator
 *
...
namespace muGrid {
    ...
    LaplaceOperator3D::LaplaceOperator3D(Real scale)
        : Parent{}, scale{scale} {}

    const GlobalFieldCollection& LaplaceOperator3D::validate_fields(
        ...
        auto* global_fc = dynamic_cast<const GlobalFieldCollection*>(
            &input_collection);
        if (!global_fc) {
            throw RuntimeError("LaplaceOperator3D requires GlobalFieldCollection");
        }

        // Check dimension matches
        if (global_fc->get_spatial_dim() != 3) {
    ...
    void LaplaceOperator3D::apply_impl(const TypedFieldBase<Real> &input_field,
    ...
    void LaplaceOperator3D::apply(const TypedFieldBase<Real> &input_field,
                                   TypedFieldBase<Real> &output_field) const {
        this->apply_impl(input_field, output_field, 1.0, false);
    }

    void LaplaceOperator3D::apply_increment(const TypedFieldBase<Real> &input_field,
                                             const Real &alpha,
                                             TypedFieldBase<Real> &output_field) const {
        this->apply_impl(input_field, output_field, alpha, true);
    }

    void LaplaceOperator3D::transpose(const TypedFieldBase<Real> &input_field,
                                       TypedFieldBase<Real> &output_field,
    ...
    void LaplaceOperator3D::transpose_increment(const TypedFieldBase<Real> &input_field,
                                                 const Real &alpha,
                                                 TypedFieldBase<Real> &output_field,
                                                 const std::vector<Real> &weights) const {
        // Laplacian is self-adjoint, so transpose = apply
    ...
    void LaplaceOperator3D::apply_impl(
    ...
    void LaplaceOperator3D::apply(
        const TypedFieldBase<Real, DefaultDeviceSpace> &input_field,
        TypedFieldBase<Real, DefaultDeviceSpace> &output_field) const {
        this->apply_impl(input_field, output_field, 1.0, false);
    }

    void LaplaceOperator3D::apply_increment(
        const TypedFieldBase<Real, DefaultDeviceSpace> &input_field,
        const Real &alpha,
        TypedFieldBase<Real, DefaultDeviceSpace> &output_field) const {
        this->apply_impl(input_field, output_field, alpha, true);

...
namespace muGrid {
    ...
    /* ---------------------------------------------------------------------- */
    Real ConvolutionOperator::apply_vecdot(
        const TypedFieldBase<Real> & nodal_field,
        TypedFieldBase<Real> & quadrature_point_field) const {
        // Apply the operator
        this->apply(nodal_field, quadrature_point_field);

        // Compute interior-only dot product: input · output
        // We need to access the collection for interior iteration
        const auto & collection = this->validate_fields(nodal_field,
                                                        quadrature_point_field);

        // Get raw data pointers
        const Real* input_data = nodal_field.data();
        const Real* output_data = quadrature_point_field.data();
        ...
        // Compute dot product over interior region only
        Real dot_product = 0.0;
        for (Index_t iz = 0; iz < params.nz; ++iz) {
            for (Index_t iy = 0; iy < params.ny; ++iy) {
                for (Index_t ix = 0; ix < params.nx; ++ix) {
                    Index_t pixel_offset = params.start_pixel_index +
                                           ix * params.nodal_stride_x +
                                           iy * params.nodal_stride_y +
                                           iz * params.nodal_stride_z;
                    for (Index_t c = 0; c < params.nodal_elems_per_pixel; ++c) {
                        dot_product += input_data[pixel_offset + c] *
                                       output_data[pixel_offset + c];
                    }
                }
            }
        }
    ...
    /* ---------------------------------------------------------------------- */
    Real ConvolutionOperator::transpose_vecdot(
        const TypedFieldBase<Real> & quadrature_point_field,
        TypedFieldBase<Real> & nodal_field,
        const std::vector<Real> & weights) const {
        // Apply transpose
        this->transpose(quadrature_point_field, nodal_field, weights);
        ...
        // Get raw data pointers
        const Real* input_data = quadrature_point_field.data();
        const Real* output_data = nodal_field.data();
        ...
        for (Index_t iz = 0; iz < params.nz; ++iz) {
            for (Index_t iy = 0; iy < params.ny; ++iy) {
                for (Index_t ix = 0; ix < params.nx; ++ix) {
                    Index_t pixel_offset = params.start_pixel_index +
                                           ix * params.quad_stride_x +
                                           iy * params.quad_stride_y +
                                           iz * params.quad_stride_z;
                    for (Index_t c = 0; c < params.quad_elems_per_pixel; ++c) {
                        dot_product += input_data[pixel_offset + c] *
                                       output_data[pixel_offset + c];
                    }
                }

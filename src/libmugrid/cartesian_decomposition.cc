...
namespace muGrid {
  CartesianDecomposition::CartesianDecomposition(
    ...
    if (nb_subdivisions.size() == 0) {
      this->collection = std::make_unqiue<GlobalFieldCollection>(
          this->nb_domain_grid_pts.size(),
      ...
      this->collection = std::make_unique<GlobalFieldCollection>(
          ...
          nb_sub_pts, StorageOrder::ArrayOfStructures, nb_ghosts_left,
          nb_ghosts_right);
    }
  }

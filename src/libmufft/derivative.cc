...
namespace muFFT {
  ...
  DiscreteDerivative::DiscreteDerivative(DynCcoord_t nb_pts,
    ...
    if (get_size(nb_pts) != stencil.size()) {
      throw DerivativeError("Number of provided data points different from "
                            "stencil size.");
    }
  ...
  DiscreteDerivative DiscreteDerivative::rollaxes(int distance) const {
    DynCcoord_t nb_pts(this->spatial_dimension),
        lbounds(this->spatial_dimension);
    Eigen::ArrayXd stencil(this->stencil.size());

    for (Dim_t dim = 0; dim < this->spatial_dimension; ++dim) {
      nb_pts[(dim + distance) % this->spatial_dimension] = this->nb_pts[dim];
      lbounds[(dim + distance) % this->spatial_dimension] = this->lbounds[dim];
    }

    for (auto && pixel : DynamicPixels(this->nb_pts, this->lbounds)) {
      DynCcoord_t rolled_pixel(this->spatial_dimension);
      for (Dim_t dim = 0; dim < this->spatial_dimension; ++dim) {
        Dim_t rolled_dim = (dim + distance) % this->spatial_dimension;
        rolled_pixel[rolled_dim] =
            pixel[dim] - this->lbounds[dim] + lbounds[dim];
      }
      stencil[get_index(nb_pts, lbounds, rolled_pixel)] =
          this->operator()(pixel);
    }

...
mkdir ${NETCDF}-build
cd ${NETCDF}-build
cmake -DENABLE_TESTS=OFF -DENABLE_HDF5=OFF -DENABLE_CDF5=OFF -DENABLE_BYTERANGE=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX ../${NETCDF}
make
make install

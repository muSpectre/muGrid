...
Three benchmark pages report CPU and GPU timings across a range of grid sizes,
...
- [Benchmark (homogenization)](benchmark_homogenization.md) — the 3D FEM
  homogenization solve (incl. MPI strong scaling), generated by
  `examples/benchmark_homogenization.py`.
- [Benchmark (preconditioner)](benchmark_homogenization_preconditioner.md) — the
  effect of the reference-material Fourier preconditioner (`-P reference`) on
  iteration count and wall time, generated by
  `examples/benchmark_homogenization_preconditioner.py`.

Re-run any of these scripts to refresh the numbers for your own hardware.

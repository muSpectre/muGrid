...
mat_vac = msp.material.MaterialLinearElastic1_2d.make(cell, "2d-vacuum", 0.0, 0)
if case == "A":
    ...
    mat_le4 = msp.material.MaterialLinearElastic4_2d.make(cell, "2d-le4-mat")
    ...
    hardening = 1e-2
    mat_hpl = msp.material.MaterialHyperElastoPlastic2_2d.make(cell, "2d-hpl")
...
for pixel_id, pixel in cell.pixels.enumerate():
    if np.array(pixel)[1] == nb_grid_pts[1]-1: #top layer in z-direction is vacuum
        ...
        if case == "A":
            ...
            mat_hpl.add_pixel(pixel_id, Young, Poisson, yield_crit[tuple(pixel)], hardening)
...
if case == "A" or case == "B":
    ...
    dec_tr_tol = 10e-2
    inc_tr_tol = 10e-4
    solver = msp.solvers.KrylovSolverTrustRegionCG(cell=cell,
                                                   tol=cg_tol,
                                                   maxiter=maxiter,
...
for frame, DelF in enumerate(DelF_list):
    if case == "A" or case == "B":
        ...
        result = msp.solvers.trust_region_newton_cg(cell, DelF, solver,
                                                    trust_region=trust_region,
                                                    newton_tol=newton_tol,
                                                    equil_tol=equil_tol,

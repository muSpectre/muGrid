...
namespace muGrid {
  ...
  BOOST_AUTO_TEST_CASE(DynamicMappedField) {
    ...
    GlobalNFieldCollection collection{twoD, NbQuad};
    collection.initialise({2, 2});
    using Mapped_t = MappedField<NFieldMap<Real, Mapping::Mut>>;
    ...
    MatrixNFieldMap<Real, Mapping::Const, NbRow, NbCol, Iteration::QuadPt>
  ...
  using ValidityDomain = NFieldCollection::ValidityDomain;
  ...
  struct FieldFixture {
    ...
    using TField_t = MappedT2NField<Real, Mapping::Mut, MDim>;
    using MField_t =
        MappedMatrixNField<Real, Mapping::Mut, SDim, MDim, Iteration::QuadPt>;
    using DField_t = RealNField;
  ...
  BOOST_FIXTURE_TEST_CASE(size_check_local,
    ...
    for (Dim_t i{0}; i < nb_pixels; ++i) {
      ...
      BOOST_CHECK_THROW(dynamic_field1.push_back(wrong_elem), NFieldError);
      BOOST_CHECK_THROW(dynamic_field2.push_back(new_elem), NFieldError);
    }
    BOOST_CHECK_EQUAL(tensor_field.get_field().size(), nb_pixels);
    BOOST_CHECK_EQUAL(dynamic_field1.size(), nb_pixels);
    BOOST_CHECK_EQUAL(dynamic_field2.size(), 0);

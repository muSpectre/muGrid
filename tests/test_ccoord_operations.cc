...
namespace muGrid {
  ...
  BOOST_AUTO_TEST_CASE(test_is_buffer_contiguous) {
    DynGridIndex nb_grid_pts2({2, 3});
    DynGridIndex column_major2({1, 2});
    DynGridIndex row_major2({3, 1});
    ...
    DynGridIndex non_contiguous2({6, 1});
    BOOST_CHECK(
        !CcoordOps::is_buffer_contiguous(nb_grid_pts2, non_contiguous2));

    DynGridIndex nb_grid_pts3({5, 3, 4});
    DynGridIndex column_major3({1, 5, 15});
    DynGridIndex row_major3({12, 4, 1});
    BOOST_CHECK(CcoordOps::is_buffer_contiguous(
  ...
  BOOST_AUTO_TEST_CASE(arithmetic_operators) {
    DynGridIndex a{3, 4};
    DynGridIndex b{1, 2};
    DynGridIndex sum{4, 6};
    DynGridIndex diff{2, 2};
    ...
    DynCoord<fourD, Real> c1{1, 2, 3};
    DynCoord<fourD, Real> c2{1.3, 2.8, 5.7};

    DynCoord<fourD, Real> c1pc2{c1[0] + c2[0], c1[1] + c2[1], c1[2] + c2[2]};
    DynCoord<fourD, Real> c1mc2{c1[0] - c2[0], c1[1] - c2[1], c1[2] - c2[2]};
